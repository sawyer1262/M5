		...
		while(1)
		{
				switch(AteStat)
				{
						...
						case 1:                          //灯全亮，全灭，跑流水
								...
								switch(AteSubStat)
								{
										...
										case 3:
												...
												break;
										case 4:
												LedVal.ledval=LED_MASK_ATE_KEPAD_CHK;
												if(KeyBk.Flag!=KeyStat.Flag || KeyBk.Numb!=KeyStat.Numb)
												{
														KeyBk=KeyStat;
														LedVal.ledval=LED_MASK_ATE_KEPAD_CHK;
						...
						case 6:           //防撬SET开关测试
						{
								...
								if(TampKeyValBk==0xFF)TampKeyValBk=EPORT_ReadGpioData(KEY_TAMPER_PIN);
								if(SetKeyValBk==0xFF)SetKeyValBk=EPORT_ReadGpioData(KEY_BACK_PIN);
								...
								if(TampKeyValBk!=EPORT_ReadGpioData(KEY_TAMPER_PIN))
								{
										...
										if(TampKeyValBk!=EPORT_ReadGpioData(KEY_TAMPER_PIN))
										{
												...
												tptdat[0]=AteStat;       //测试项
												tptdat[1]=1;
												tptdat[2]=TampKeyValBk;    //测试值
												UartBack_SendCmdData(ATE_UPLINK,tptdat,3,1,1000);
										}
								}
						...
						case 7:          //PIR测试
						{
								...
								if(PirValBk==0xFF)PirValBk=CPM_ReadGpioData(CPM_WAKEUP_PIN);
						...
						case 16:           //静态电流测试 
								switch(AteSubStat)
								{
										case 0:
												...
												//指纹进入低功耗，以及应答发送完成
												if((LPM_GetStopModeValue()&LPM_BACKLOCK_TX_ID)==0 && FpPowerDown==1)
												{
														AteSubStat++;
												}
												break;
